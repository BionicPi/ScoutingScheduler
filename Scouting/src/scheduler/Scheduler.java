...
public class Scheduler {
	...
	public class Scouter {
		...
		void sortTeams()
		{
			final ArrayList<Team> temp = new ArrayList<Team>();
	...
	public void parseMatchList(String list)
	{
		...
		{
			final int b1 = Integer.parseInt(list.substring(i, list.indexOf(",", i)));
			i = list.indexOf(",", i) + 1;
			final int b2 = Integer.parseInt(list.substring(i, list.indexOf(",", i)));
			i = list.indexOf(",", i) + 1;
			final int b3 = Integer.parseInt(list.substring(i, list.indexOf(",", i)));
			i = list.indexOf(",", i) + 1;
			final int r1 = Integer.parseInt(list.substring(i, list.indexOf(",", i)));
			i = list.indexOf(",", i) + 1;
			final int r2 = Integer.parseInt(list.substring(i, list.indexOf(",", i)));
			i = list.indexOf(",", i) + 1;
			final int r3 = Integer.parseInt(list.substring(i, list.indexOf(",", i)));
			i = list.indexOf(",", i) + 1;
			
			// System.out.println(b1.getTeamNumber() + " " + b2.getTeamNumber()
			// + " " + b3.getTeamNumber() + " " + r1.getTeamNumber() + " " +
			// r2.getTeamNumber() + " " + r3.getTeamNumber() + "");
		...
		for (final Match m : matchList)
			for (final int t : m.numbers)
				if (!teamNumbers.contains(t))
					teamNumbers.add(t);
				
		for (final int i : teamNumbers)
	...
	public void clearScouterOverlap()
	{
		for (int i = 0; i < (5 * scouters.size()); i++)
		{
			...
			for (int j = 0; j < (scouters.get(i % scouters.size()).matches.size() - 1); j++)
				if (scouters.get(i % scouters.size()).matches.get(j)
						.getMatchNumber() == scouters.get(i % scouters.size()).matches.get(j + 1).getMatchNumber())
					scouters.get((i + 1) % scouters.size()).matches
							.add(scouters.get(i % scouters.size()).matches.remove(j + 1));
		}
	}
	...
	{
		scouters.add(new Scouter(s));
	}
	
	public static void main(String[] args)
	{
		final Scheduler VAHAY = new Scheduler();
		...
		try
		{
			...
			final FileReader file = new FileReader("MatchSchedule.csv");
			final BufferedReader bFile = new BufferedReader(file);
			...
			for (final Scouter s : VAHAY.scouters)
				s.filterMatches();
